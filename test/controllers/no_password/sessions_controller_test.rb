...
module NoPassword
  class SessionsControllerTest < ActionDispatch::IntegrationTest
    test "it creates a session with nil referer_path if return_to path is engine's new_session_path" do
      ...
      post no_password.session_path(return_to: CGI.escape(request.fullpath)), params: {session: {email: "ana@example.com"}}, headers: {HTTP_USER_AGENT: "Mozilla/5.0"}
    ...
    test "it creates a session with referer_path if return_to path is an internal url" do
      path = secure_area_path
      get path
      post no_password.session_path(return_to: CGI.escape(path)), params: {session: {email: "ana@example.com"}}, headers: {HTTP_USER_AGENT: "Mozilla/5.0"}
    ...
    test "it redirects to no_password.edit_session_confirmations_path after send form with email" do
      post no_password.session_path, params: {session: {email: "ana@example.com"}}, headers: {HTTP_USER_AGENT: "Mozilla/5.0"}
